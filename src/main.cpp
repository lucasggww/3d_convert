...
/*
 IVCON changelog:
	31 May 2008	        TL Fixed WRL, added COB write support
...
void hello()
{
	...
	printf(".cob           Caligari trueSpace object;\n");
#endif
...
int command_execute(char *input)
{
	...
	if (strncmp(comand, "TA", 2) == 0)
	{
		...
		if (new_tiny_area < 0.f)
		{
			...
			printf("INTERACT - Command execution skipped!\n");
			printf(
					"  Cannot find nonnegative floating point number in input data.\n");
			return -2;
		}
		printf("\n");
		...
		printf(
				"  The larger value you type, the more faces will be deleted.\n");
	...
	if (strncmp(comand, "TS", 2) == 0)
	{
		...
		if (isnan(x) || isnan(y) || isnan(z))
		{
			...
			printf("INTERACT - Command execution skipped!\n");
			printf(
					"  Cannot find 3 proper floating point factors in input data.\n");
			return -2;
		}
		printf("\n");
