...
#define _ivcon_version "1.18 TL (19 Apr 2008)"
...
protected:	// converters
	...
	bool txt_write(FileIO *fileout);
	bool ucd_write(FILE *fileout);
	bool vla_read(FileIO *filein);
	bool vla_write(FileIO *fileout);
	bool wrl_write(FileIO *fileout);
	bool xgl_write(FileIO *fileout);
...
protected:
  ...
  // CONVERTERS ADDITIONAL FUNCTION PROTOTYPES
    ...
    bool wrl_write_shapes(FileIO *fileout);
    bool wrl_write_mesh(FileIO *fileout, unsigned long mesh_idx);
    bool wrl_write_lineset(FileIO *fileout);
    bool wrl_write_faceset(FileIO *fileout, unsigned long mesh_idx);

    // low-level XGL write function;
    bool xgl_write_object(FileIO *fileout, unsigned long mesh_idx);
    bool xgl_write_mesh(FileIO *fileout, unsigned long mesh_idx);
    bool xgl_write_material(FileIO *fileout, unsigned long mat_idx);
    bool xgl_write_lghtng(FileIO *fileout);
    bool xgl_write_bkgnd(FileIO *fileout);
